...
export const handleAddSelected = (ctx) => {
  ...
  switch (addCardType) {
    ...
    case 'cover_panel': {
      const coverPanelCardId = `cover_panel_card_${Date.now()}`;
      commitSingleCard(coverPanelCardId, {});
      return;
    }

    case 'security_panel': {
      const securityPanelCardId = `security_panel_card_${Date.now()}`;
      commitSingleCard(securityPanelCardId, {});
      return;
    }
