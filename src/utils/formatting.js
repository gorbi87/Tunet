export const formatRelativeTime = (timestamp, t) => {
  if (!timestamp || timestamp === 'unavailable' || timestamp === 'unknown' || timestamp === '--')
    return '--';
  ...
  try {
    ...
    return translate('time.hoursMinutesAgo')
      .replace('{hours}', diffHours)
      .replace('{minutes}', remainingMins);
  } catch (_e) {
